...
void Timer1_Init()
{
	...
	TMOD = TMOD|0X10; // Change low 4 to 0001
	
	// Set up TCON (Can be done by bit)
	TF1 = 0; // Clear the overflow flag
	TR1 = 1; // Allow the counter to work
...
void Timer0_Routine(void) interrupt 3
{
	static unsigned int T1Count; // Static is not destroyed
	// Fix the initialization back
	TL1 = 0x66;		//Initial timer value
